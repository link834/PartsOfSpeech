...
def viterbi(sentence, hmm):
	...
	for k in range(1, n+1):
		for u in partsOfSpeech(k-1):
			for v in partsOfSpeech(k):
				backPointers[k,u,v], pi[k,u,v] = argmax([(word, pi[k-1, word, u] * threeProb(v, word, u) * emissionProb(observations[k], v)) for word in partsOfSpeech(k-2)])

	(actualStates[n-1], actualStates[n]), score = argmax([((u,v), pi[n, u, v] * threeProb("STOP", u, v)) for u in partsOfSpeech(n-1) for v in partsOfSpeech(n)])
